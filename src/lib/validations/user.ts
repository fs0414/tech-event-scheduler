...
export const addOwnerSchema = z.object({
  ...
  role: z
    .number()
    .int()
    .refine((val) => val === OWNER_ROLES.ADMIN || val === OWNER_ROLES.MEMBER, {
      message: "無効なロールです",
    })
...
export const validateSearchUser = (
  data: unknown,
): ValidationResult<SearchUserInput> => {
  try {
...
export const validateAddOwner = (
  data: unknown,
): ValidationResult<AddOwnerInput> => {
  try {
...
export const validateChangeOwnerRole = (
  data: unknown,
): ValidationResult<ChangeOwnerRoleInput> => {
  try {
