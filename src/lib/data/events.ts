...
export const getEventWithOwnership = cache(
  async (id: number, userId?: string) => {
    ...
    const isOwner = userId
      ? event.owners.some((owner) => owner.userId === userId)
      : false;
...
export const checkEventOwnership = cache(
  async (userId: string, eventId: number): Promise<boolean> => {
