...
export default function EventCreateClient({
  ...
  const [searchResult, setSearchResult] = useState<Pick<
    User,
    "id" | "name" | "email"
  > | null>(null);
  ...
  const handleEmailSearch = (value: string) => {
    ...
    startTransition(async () => {
      const excludeUserIds = [
        currentUser.id,
        ...selectedOwners.map((owner) => owner.id),
      ];
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-4 sm:py-6 lg:py-8 max-w-4xl mx-auto">
        ...
        <div className="mb-4 sm:mb-6">
          ...
          <nav className="flex items-center gap-1 text-xs sm:text-sm flex-wrap">
            ...
            <span className={createTypographyClasses("xs", "regular", "muted")}>
              /
            </span>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)}>
            ...
            >
              <CardHeader>
                <CardTitle
                  className={createTypographyClasses("l", "bold", "dark")}
                >
                  イベント情報
                </CardTitle>
              </CardHeader>
