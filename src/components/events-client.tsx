...
export default function EventsClient({
  ...
  const totalUsers = new Set(
    events
      .flatMap((event) => (event.owners || []).map((owner) => owner.userId))
      .filter(Boolean),
  ).size;
  const totalOwners = events.reduce(
    (sum, event) => sum + (event.owners?.length || 0),
    0,
  );
  const totalSpeakers = events.reduce(
    (sum, event) => sum + (event.speakers?.length || 0),
    0,
  );
  const totalTimers = events.reduce(
    (sum, event) => sum + (event.timers?.length || 0),
    0,
  );
    ...
    >
      ...
      <div className={cn(UI_CONSTANTS.spacing.marginBottom)}>
        ...
        >
          ...
          >
            ...
            <span className={createTypographyClasses("s", "semibold", "body")}>
              新しいイベント
            </span>
          </Button>
        </div>
          ...
          <CardContent className={cn(UI_CONSTANTS.spacing.sectionPadding)}>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="text-center">
                <div className={createTypographyClasses("xl", "bold", "muted")}>
                  {events.length}
                </div>
                <div
                  className={createTypographyClasses("s", "regular", "muted")}
                >
                  総イベント数
                </div>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredEvents.map((event) => {
          ...
          const totalParticipants =
            eventOwners.length + eventSpeakers.length + eventTimers.length;
            ...
            >
              <CardContent className={cn("p-0")}>
                ...
                {eventOwners.length > 0 && (
                  <div
                    className={cn(
                      "mx-6 mb-4 p-4 rounded-lg",
                      UI_CONSTANTS.colors.paleAccentBg,
                    )}
                  >
                    <div className="mb-3">
      ...
      {filteredEvents.length === 0 && (
        <div className="text-center py-12">
          ...
          >
            <Calendar
              className={cn("w-12 h-12", UI_CONSTANTS.colors.mutedText)}
            />
          </div>
          <h3 className={createTypographyClasses("l", "medium", "body")}>
            イベントが見つかりません
          </h3>
          <p
            className={cn(
              "mb-4",
              createTypographyClasses("m", "regular", "secondary"),
            )}
          >
            検索条件を変更してお試しください
          </p>
