...
import type { Event, User, Owner } from '@prisma/client';

type EventWithOwners = Event & {
  owners: (Owner & {
    user: User;
  })[];
};
...
export default function EventsClient({ events, initialSearch = '' }: EventsClientProps) {
  ...
  const totalUsers = new Set(events.flatMap(event => event.owners.map(owner => owner.userId))).size;
  const totalOwners = events.reduce((sum, event) => sum + event.owners.length, 0);
